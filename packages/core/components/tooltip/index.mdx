...
Currently Tooltip accepts a string as its child.

Components that represent singleton elements (eg, `<img>`, `<input>`) cannot use Tooltip.

Future work: Accept components as children. Consider alternative backing libraries
(currently Tooltip.js, from Popper.js).
...
<Playground>
    ...
    <Box my={8}>
        <Button data-tooltip>
            Click me
            <Tooltip trigger="click" hasArrow placement="bottom">Ahoy there!</Tooltip>
        </Button>
    </Box>

    <Box my={8}>
        <Swatch color="red" size="2em" data-tooltip>
            <Tooltip hasArrow placement="right">Red</Tooltip>
        </Swatch>
    </Box>
